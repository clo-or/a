...
primary_button = st.button('케이팝 데몬 헌터스 헌트릭스 노래', type='primary')
if primary_button:
...
st.text('음식(떡볶이, 김밥, 라면)등이 대체적으로 이어져있고, 라이프스타일, 전통, 호랑이, 관광등이 형성되어있는 것을 보아, 대표적인 한국 라이프스타일과 문화를 볼 수 있는 영화가 팬덤 형성의 핵심 요인이 되었다는 것을 확인할 수 있다.')st.button('네트워크 시각화 이미지 기본'):
